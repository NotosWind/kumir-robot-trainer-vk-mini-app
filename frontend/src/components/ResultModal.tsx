...
function failureReason(result: CheckResult): string {
  ...
  if (first && first.missing > 0 && first.extra === 0) {
    return `Закрашены не все нужные клетки — осталось пустыми: ${first.missing}. Частая ошибка: в цикле «нц пока» последняя клетка остаётся незакрашенной. Закрась её до цикла или добавь «закрасить» после цикла.`
  }
